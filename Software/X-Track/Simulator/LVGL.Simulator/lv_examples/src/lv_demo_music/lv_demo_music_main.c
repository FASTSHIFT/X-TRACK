...
//static lv_res_t main_cont_Call the ancestor's event handler_cb(lv_obj_t * obj, lv_Call the ancestor's event handler_t Call the ancestor's event handler, void * param);
...
//static lv_Call the ancestor's event handler_cb_t ancestor_Call the ancestor's event handler_cb;
...
//    lv_obj_set_size(main_cont, LV_HOR_RES, LV_VER_RES * 2 + 30);
//#else
...
//    ancestor_Call the ancestor's event handler_cb = lv_obj_get_Call the ancestor's event handler_cb(main_cont);
//    lv_obj_set_Call the ancestor's event handler_cb(main_cont, main_cont_Call the ancestor's event handler_cb);
//
...
//#if LV_DEMO_MUSIC_SQUARE
//    lv_obj_set_width(title_box, LV_HOR_RES);
//    lv_obj_align(title_box, NULL, LV_ALIGN_IN_TOP_MID, 0, LV_VER_RES + 5);
//#elif LV_DEMO_MUSIC_LANDSCAPE
...
//#if LV_DEMO_MUSIC_SQUARE
//    lv_obj_align(icon_box, title_box, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
//#elif LV_DEMO_MUSIC_LANDSCAPE
...
//#if LV_DEMO_MUSIC_SQUARE
//    lv_obj_set_size(spectrum_obj, LV_HOR_RES, LV_VER_RES);
//    lv_obj_align(spectrum_obj, NULL, LV_ALIGN_IN_TOP_MID, 0, 10);
//#elif LV_DEMO_MUSIC_LANDSCAPE
...
//            lv_anim_set_values(&a, lv_obj_get_y(main_cont), -LV_VER_RES - 7);
//            lv_anim_start(&a);
//        }
//#endif
//    }
...
//{
//    lv_res_t res;
//    res = ancestor_Call the ancestor's event handler_cb(obj, Call the ancestor's event handler, param);
//    if(res != LV_RES_OK) return res;
//
//    if(Call the ancestor's event handler == LV_SIGNAL_DRAG_THROW_BEGIN) {
...
//        lv_anim_start(&a);
//    }
//    else if(Call the ancestor's event handler == LV_SIGNAL_COORD_CHANGED) {
//        lv_coord_t y = lv_obj_get_y(obj);
