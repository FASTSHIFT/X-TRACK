...
void lv_multi_line::start()
{
    ...
    {
        LV_LOG_INFO("current_index[%d] >= multi_line.size()[%d]", current_index, multi_line.size());
        add_line();
        refresh_line();
    }

    LV_LOG_INFO("show multi_line[%d]", current_index);
...
void lv_multi_line::append(const lv_point_t* point)
{
    ...
    {
        LV_LOG_WARN("multi_line.size() = 0");
        return;
    }

    LV_LOG_INFO("multi_line[%d]: (%d, %d)", current_index, point->x, point->y);
...
void lv_multi_line::stop()
{
    ...
    LV_LOG_INFO("multi_line[%d] size = %d", current_index, single_line->points.size());
    current_index++;
}
...
void lv_multi_line::reset()
{
    current_index = 0;
    size_t size = multi_line.size();
    LV_LOG_INFO("multi_line.size() = %d", size);
